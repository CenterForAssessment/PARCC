#####################################################################################
###                                                                               ###
###      SGP Configurations for Fall 2015 EOCT Math subjects  -  SCALE SCORES     ###
###                                                                               ###
#####################################################################################

ALGEBRA_I_SS.2015_2016.1.config <- list(
	ALGEBRA_I_SS.2015_2016.1 = list(
		sgp.content.areas=c("MATHEMATICS_SS", "ALGEBRA_I_SS"),
		sgp.panel.years=c("2014_2015.2", "2015_2016.1"),
		sgp.grade.sequences=list(c("8", "EOCT")), # 9th grade
		sgp.projection.grade.sequences=list("NO_PROJECTIONS"))
)

GEOMETRY_SS.2015_2016.1.config <- list(
	GEOMETRY_SS.2015_2016.1 = list(
		sgp.content.areas=c("ALGEBRA_I_SS", "GEOMETRY_SS"),
		sgp.panel.years=c("2014_2015.2", "2015_2016.1"),
		sgp.grade.sequences=list(c("EOCT", "EOCT")),
		sgp.projection.grade.sequences=list("NO_PROJECTIONS"))
)

ALGEBRA_II_SS.2015_2016.1.config <- list(
	ALGEBRA_II_SS.2015_2016.1 = list(
		sgp.content.areas=c("GEOMETRY_SS", "ALGEBRA_II_SS"),
		sgp.panel.years=c("2014_2015.2", "2015_2016.1"),
		sgp.grade.sequences=list(c("EOCT", "EOCT")),
		sgp.projection.grade.sequences=list("NO_PROJECTIONS"))
)
