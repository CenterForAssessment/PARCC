...
NJSS_Data_LONG_2021_2022.1 <-
    freadZIP("Data/Base_Files/njssf21_NJ_State_Summative_Record_File_StartStrong_D202111081433.csv.zip")
# nj1 <- fread("./Data/Base_Files/NJ Start Strong Summative/njssf21_NJ_State_Summative_Record_File_StartStrong_D202111081433_File1.csv", colClasses=rep("character", 201))
# nj2 <- fread("./Data/Base_Files/NJ Start Strong Summative/njssf21_NJ_State_Summative_Record_File_StartStrong_D202111081433_File2.csv", colClasses=rep("character", 201))

# NJSS_Data_LONG_2021_2022.1 <- rbindlist(list(nj1, nj2)); rm(nj1); rm(nj2); gc()

##    Weed out blank columns and unused TestCode records
NJSS_Data_LONG_2021_2022.1 <-
    NJSS_Data_LONG_2021_2022.1[!TestCode %in% c("SC06", "SC09", "SC12")]
NJSS_Data_LONG_2021_2022.1 <-
    NJSS_Data_LONG_2021_2022.1[,
      names(NJSS_Data_LONG_2021_2022.1)[
        !unlist(lapply(names(NJSS_Data_LONG_2021_2022.1),
                       \(f) all(NJSS_Data_LONG_2021_2022.1[, get(f)] == "")))
      ], with = FALSE
    ]

setnames(NJSS_Data_LONG_2021_2022.1,
         c("StudentAssessmentIdentifier", "TestRawScore", "SupportLevel"),
         c("ID",  "SCALE_SCORE_ACTUAL", "ACHIEVEMENT_LEVEL_ACTUAL"))
NJSS_Data_LONG_2021_2022.1[, StateAbbreviation := "NJ"]

NJSS_Data_LONG_2021_2022.1[,
    SCALE_SCORE_ACTUAL := as.numeric(SCALE_SCORE_ACTUAL)]
NJSS_Data_LONG_2021_2022.1[,
    ReportingConcept1RawScore := as.numeric(ReportingConcept1RawScore)]
NJSS_Data_LONG_2021_2022.1[,
    ReportingConcept2RawScore := as.numeric(ReportingConcept2RawScore)]
NJSS_Data_LONG_2021_2022.1[,
    ReportingConcept3RawScore := as.numeric(ReportingConcept3RawScore)]
NJSS_Data_LONG_2021_2022.1[,
    ReportingConcept4RawScore := as.numeric(ReportingConcept4RawScore)]


###   Create VALID_CASE based on business rules
NJSS_Data_LONG_2021_2022.1[, VALID_CASE := "VALID_CASE"]
NJSS_Data_LONG_2021_2022.1[VoidScoreCode == "Y" |
                           NotTestedCode == "Y" |
                           TestAttemptednessFlag == "N",
                               VALID_CASE := "INVALID_CASE"]
NJSS_Data_LONG_2021_2022.1[is.na(SCALE_SCORE_ACTUAL),
                               VALID_CASE := "INVALID_CASE"]
...
setattr(NJSS_Data_LONG_2021_2022.1$CONTENT_AREA,
        "levels",
        c("ALG_I_NJSS", "ALG_II_NJSS",
          rep("ELA_NJSS", 7),
          "GEOM_NJSS",
          rep("MATH_NJSS", 5))
       )
...
`princompLong` =
  function(
    ...
    tmp.data <- data_table[, mget(vars_to_use)][,
        vars_to_use[!unlist(lapply(vars_to_use,
                                   \(f) all(is.na(data_table[, get(f)]))
                                   )
                            )
                    ], with = FALSE]
    round(as.numeric(psych::principal(tmp.data)[["scores"]]), digits)#, cor="poly", nfactors = 1)
  }
...
component.scores <-
  c("ReportingConcept1RawScore",
    "ReportingConcept2RawScore",
    "ReportingConcept3RawScore",
    "ReportingConcept4RawScore"
  )

NJSS_Data_LONG_2021_2022.1[,
    SCALE_SCORE := princompLong(.SD, vars_to_use = component.scores),
    by = list(TestCode),
    .SDcols = c("VALID_CASE", component.scores)
]
...
`getCutsNJSS` =
  function(
    ...
    tmp.dt <-
      data_table[ACHIEVEMENT_LEVEL_ACTUAL != "",
                 list(MIN = min(SCALE_SCORE_ACTUAL),
                      MAX = max(SCALE_SCORE_ACTUAL)
                     ),
                 keyby = "ACHIEVEMENT_LEVEL_ACTUAL"]

    tmp.data <-
      data.table(SCALE_SCORE_ACTUAL =
                 c(unlist(floor(tmp.dt[1, 3, ]/2)),
                   unlist(tmp.dt[2:3, 2]),
                   ceiling(mean(unlist(c(tmp.dt[3, 2], tmp.dt[3, 3]))))
                 ))
    if (predict.cuts) {
      tmp.cuts <- round(predict(tmp.lm, tmp.data), digits)
    } else {
      tmp.cuts <- as.numeric(tmp.data[, SCALE_SCORE_ACTUAL])
    }
    as.character(
      factor(findInterval(data_table[, SCALE_SCORE], tmp.cuts) + 1,
             levels = 1:5,
...
NJSS_Data_LONG_2021_2022.1[,
  ACHIEVEMENT_LEVEL := getCutsNJSS(.SD),
  by = list(TestCode),
  .SDcols = c("SCALE_SCORE", "SCALE_SCORE_ACTUAL", "ACHIEVEMENT_LEVEL_ACTUAL")
]
...
NJSS_Data_LONG_2021_2022.1 |>
       setkey(VALID_CASE, CONTENT_AREA, YEAR, GRADE, ID, SCALE_SCORE) |>
       setkey(VALID_CASE, CONTENT_AREA, YEAR, GRADE, ID)
dupl <- duplicated(NJSS_Data_LONG_2021_2022.1, by = key(NJSS_Data_LONG_2021_2022.1))
# dups <-
#   data.table(NJSS_Data_LONG_2021_2022.1[unique(c(which(dupl)-1, which(dupl))), ],
#              key = key(NJSS_Data_LONG_2021_2022.1))
# table(dups$VALID_CASE) # 5 duplicates that are VALID_CASEs
# table(NJSS_Data_LONG_2021_2022.1[which(dupl)-1, is.na(SCALE_SCORE), GRADE])
NJSS_Data_LONG_2021_2022.1[which(dupl) - 1, VALID_CASE := "INVALID_CASE"]
      ...
      if (denom > 9) {
        ...
        if (denom > 2) {
          tmp.deci <-
            as.numeric(
              as.character(
                factor(vals, levels = tmp.pts,
                       labels = c(0, round(((2:(denom-1))/9)*(9/(denom+1)), 1), 0.9))
            ))
          # tmp.deci <- # vals |>
          #   factor(levels = tmp.pts,
          #          labels = c(0, round(((2:(denom - 1)) / 9) * (9 / (denom + 1)), 1), 0.9)) |>
          #   as.character() |> as.numeric()
        } else { # denom == 2
          tmp.deci <- # vals |>
            as.numeric(as.character(factor(vals, levels = tmp.pts, labels = c(0, 0.5))))
            # factor(levels = tmp.pts, labels = c(0, 0.5)) |>
            #   as.character() |> as.numeric()
        }
...
NJSS_Data_LONG_2021_2022.1[,
    SCALE_SCORE_RANKED := quantileOrder(.SD),
    by = list(TestCode, SCALE_SCORE_ACTUAL),
    .SDcols = c("SCALE_SCORE", "SCALE_SCORE_ACTUAL")
] # "TestCode" for debug

setnames(NJSS_Data_LONG_2021_2022.1,
         c("SCALE_SCORE", "SCALE_SCORE_RANKED"),
         c("SSX", "SCALE_SCORE"))
NJSS_Data_LONG_2021_2022.1[,
    ACHIEVEMENT_LEVEL_RANKED := getCutsNJSS(.SD, predict.cuts = FALSE),
    by = list(TestCode),
    .SDcols = c("SCALE_SCORE", "SCALE_SCORE_ACTUAL", "ACHIEVEMENT_LEVEL_ACTUAL")
]
...
#                              keyby = c("TestCode", "SCALE_SCORE_ACTUAL", "SCALE_SCORE_RANKED")]
...
parcc.var.names <-
  c("AssessmentYear", "StateAbbreviation", "PANUniqueStudentID",
    ...
    "IRTTheta", "SummativeCSEM", "ThetaSEM",
...
added.sgp.names <-
  c("ID", "VALID_CASE", "CONTENT_AREA", "GRADE", "YEAR",
...
vars.to.keep <-
  c(parcc.var.names[parcc.var.names %in% names(NJSS_Data_LONG_2021_2022.1)],
    "StudentUUID", "TestingOrganizationalType"
  )
vars.to.remove <-
  names(NJSS_Data_LONG_2021_2022.1)[
    !names(NJSS_Data_LONG_2021_2022.1) %in%
      c(added.sgp.names, unique(c(parcc.var.names, vars.to.keep)))
  ]

NJSS_Data_LONG_2021_2022.1[, (vars.to.remove) := NULL]

setcolorder(NJSS_Data_LONG_2021_2022.1,
            c(added.sgp.names, vars.to.keep))

###   Create Data Directory and Save Object
if (!dir.exists("./Data/Archive/2021_2022.1"))
     dir.create("./Data/Archive/2021_2022.1", recursive = TRUE)

save(NJSS_Data_LONG_2021_2022.1,
     file = "./Data/Archive/2021_2022.1/NJSS_Data_LONG_2021_2022.1.Rdata")
...
#     UNIQUE_RAWS = length(unique(SCALE_SCORE_ACTUAL))
#   ), keyby = "TestCode"]
...
#
# require(lattice)
# xyplot(SCALE_SCORE ~ SCALE_SCORE_RANKED|TestCode,
#        data = NJSS_Data_LONG_2021_2022.1,
#        main = "Scatterplots of PCA vs Ranked Raw Scores",
#        ylab = "PCA", xlab = "Augmented Raw Scores")

# xyplot(SCALE_SCORE_ACTUAL ~ SCALE_SCORE_RANKED|TestCode,
#        data = NJSS_Data_LONG_2021_2022.1,
