...
for (org in c("Bureau_of_Indian_Education", "Department_Of_Defense",
              "Illinois", "New_Jersey", "PARCC")) {
    load(
        paste0(org, "/Data/Archive/2021_2022.2/",
...
State_LONG_Data <-
    rbindlist(
        list(Bureau_of_Indian_Education_SGP_LONG_Data_2021_2022.2,
...
state.tmp.split <-
    strsplit(
        as.character(State_LONG_Data$SGP_NORM_GROUP),
        "; "
    )
...
setattr(
    State_LONG_Data$CONTENT_AREA_1PRIOR,
    "levels",
    c(NA, "ALG01", "ALG02", "ALG01", "ELA", "ELA", "GEOM", "GEOM", "MAT", "MAT")
)
# levels(State_LONG_Data$CONTENT_AREA_1PRIOR) <-
#     c(NA, "ALG01", "ALG02", "ELA", "ELA", "GEOM", "MAT", "MAT")
...
State_LONG_Data[GRADE_1PRIOR == "EOCT",
                  GRADE_1PRIOR := ""
]
State_LONG_Data[GRADE_1PRIOR %in% 3:9,
                  GRADE_1PRIOR := paste0("0", GRADE_1PRIOR)
]
...
state.score.split <-
    strsplit(
        as.character(State_LONG_Data$SGP_NORM_GROUP_SCALE_SCORES),
        "; "
    )
...
# Only IL had (state) projections in 2022
...
Parcc_LONG_Data[, SGPIRTThetaConsortia1Prior :=
    as.numeric(
        sapply(parcc.score.split, function(x) rev(x)[2])
    )
]
