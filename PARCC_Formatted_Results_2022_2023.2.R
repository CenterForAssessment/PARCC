###############################################################################
###                                                                         ###
###    Format Spring 2023 Consortium Results to Pearson's specifications    ###
###                                                                         ###
###############################################################################
...
###   Read in Spring 2023 Output Files
for (org in c("Bureau_of_Indian_Education", "Department_Of_Defense",
              "Illinois", "New_Jersey", "Washington_DC", "PARCC")) {
    load(
        paste0(org, "/Data/Archive/2022_2023.2/",
               gsub("Of", "of", org), "_SGP_LONG_Data_2022_2023.2.Rdata"
        )
    )
}
...
Illinois_SGP_LONG_Data_2022_2023.2[!is.na(SGP_TARGET_3_YEAR),
...
# assign("State_LONG_Data", Department_of_Defense_SGP_LONG_Data_2022_2023.2)
# rm(Department_of_Defense_SGP_LONG_Data_2022_2023.2); gc()

assign("PARCC_LONG_Data", PARCC_SGP_LONG_Data_2022_2023.2)
rm(list = grep("2022_2023", ls(), value = TRUE)); gc()
...
setattr(State_LONG_Data$CONTENT_AREA_2PRIOR, "levels",
        c(NA, "ALG01", "ELA", "MAT")) # DoDEA, ALL
        # c(NA, "ELA", "MAT"))
# State_LONG_Data[, CONTENT_AREA_2PRIOR := NA] # NJ (added after DC)
...
# State_LONG_Data[,
#     SGPPreviousTestCodeState := gsub("NA;", "", SGPPreviousTestCodeState)
# ] # DC
...
#          "_2022-2023_Spring_SGP-STATE_LEVEL_Results_",
#          format(Sys.Date(), format = "%Y%m%d"), ".csv"
#         )
# if (abv == "DD") fname <- gsub("_of_", "_Of_", fname) # DoDEA folder name
# if (!abv %in% c("DD", "BI")) FINAL_LONG_Data[, (addl.bie.dd.names) := NULL]
# options(scipen = 999)
...
####  format that it was received and return requested
tmp.wd <- getwd()
options(scipen = 999)
for (abv in unique(FINAL_LONG_Data$StateAbbreviation)) {
  fname <-
      paste0("./",
             gsub(" ", "_", SGP:::getStateAbbreviation(abv, type = "state")),
             "/Data/Pearson/PARCC_", abv, "_2022-2023_Spring_SGP-Results_",
             format(Sys.Date(), format = "%Y%m%d"), ".csv")
  if (abv == "DD") fname <- gsub("_of_", "_Of_", fname) # DoDEA folder name
  if (!abv %in% c("DD", "BI")) {
