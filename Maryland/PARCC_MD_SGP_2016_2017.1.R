...
ELA_SS_2016_2017.1.config <- list(
	ELA_SS.2016_2017.1 = list(
...
ELA_2016_2017.1.config <- list(
	ELA.2016_2017.1 = list(
...
Maryland_SGP <- abcSGP(
		...
		sgp_object=rbindlist(list(
			...
			dbGetQuery(dbConnect(SQLite(), dbname = parcc.db), "select * from PARCC_Data_LONG_2017_1 where StateAbbreviation in ('MD')")), fill=TRUE),
...
if (sgp.test) {
	save(Maryland_SGP, file="./Data/SIM/Maryland_SGP-Test_PARCC_2016_2017.1.Rdata")
} else save(Maryland_SGP, file="./Data/Archive/2016_2017.1/Maryland_SGP.Rdata")
