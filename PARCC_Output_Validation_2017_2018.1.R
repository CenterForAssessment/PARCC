...
###                                                                          ###
###                                                                          ###
###                   Validate Fall 2017 PARCC SGP output                    ###
###                                                                          ###
###                                                                          ###
################################################################################

cat("#########################################\n", file="PARCC_Output_Validation_2017_2018.1.out")
...
for (state in tmp.states) {
    ...
    cat(paste("\n### Test for Identical PARCCStudentIdentifier in BASE and OUTPUT files:", tmp.tf), file="PARCC_Output_Validation_2017_2018.1.out", append=TRUE)

    tmp.sgp.state <- paste(as.character(summary(as.numeric(tmp.OUTPUT$StudentGrowthPercentileComparedtoState))), collapse=" ")
    cat(paste("\n### Test of StudentGrowthPercentileComparedtoState:", tmp.sgp.state), file="PARCC_Output_Validation_2017_2018.1.out", append=TRUE)
    ...
    capture.output(tmp.simex.state, file = "PARCC_Output_Validation_2017_2018.1.out", append=TRUE)
    ...
    capture.output(tmp.simex.parcc, file = "PARCC_Output_Validation_2017_2018.1.out", append=TRUE)
