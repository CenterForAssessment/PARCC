...
# Measurement Error Impact and Correction in SGPs

Error-prone standardized test scores are used as covariates in the quantile regression models used to produce SGPs.  This violates a fundamental assumption of all regression models that the error term of the model is independent of covariates [c.f., @Fuller:2006]. When a regression covariate is measured with error, measurement error (ME) combines with the regression residual to form a composite model residual. The error-prone covariate is then correlated with the model residual since it is correlated with its own ME [@LordNovick]. The correlation is a symptom of what is known as endogeneity, and it not only causes bias in the estimated coefficient of the error-prone covariate, but can also affect all other coefficients in the model [@Fuller:2006]. While there is an extensive literature about the endogeneity problem in classical regression and its implications for value-added and other related models [see, for example, @Fuller:2006; @Goldstein:1995; @battauz2011covariate; @MccaLock:2014], the issue has not attracted as much attention in the context of quantile regression [@Chesher:2001; Schennach:2008; @WeiYingCarroll:2009]. The impact of covariate ME in the computation of SGPs at the individual level and aggregated levels have recently been explored [@ShangVanIBet:2015]. 
