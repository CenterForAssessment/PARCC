...
subtitle: "A Technical Overview of the Spring 2018 Student Growth Percentile Calculations"
author:
...
This document was written by Damian Betebenner & Adam VanIwaarden for the PARCC consortium and Pearson Education.
	...
	Original Draft:  October 31, 2018
	Final Draft:     November 30, 2018
-->


```{r, echo=FALSE, include=FALSE}
  ## set a universal Cache path
  knitr::opts_chunk$set(cache.path = "_cache/PARCC_SGP_Spr2018")
...
- ***SGP Results - *** provides basic descriptive statistics from the 2018 analyses at the consortium and state levels.

This report also includes multiple appendices.  Appendix A displays Goodness of Fit plots for each analysis conducted for Spring 2018.  Appendix B provides a more comprehensive discussion of the SGP methodology.  Appendix C is an investigation of potential ceiling and/or floor effects present in the PARCC consortium assessment data and growth analyses.
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_EOG <- table(PARCC_SGP@Data[YEAR=='2017_2018.2' & CONTENT_AREA %in% GL_subjects & VALID_CASE=='VALID_CASE']$CONTENT_AREA, PARCC_SGP@Data[YEAR=='2017_2018.2' & CONTENT_AREA %in% GL_subjects & VALID_CASE=='VALID_CASE']$GRADE)
```
```{r, results='asis', echo=FALSE, N_tableEOG}
  ...
  cat(dualTable(as.matrix(n_tbl_EOG_B), 
  	align=paste(rep('r', dim(n_tbl_EOG_B)[2]), collapse=''),
		...
		caption='Number of Valid Grade Level Student Records by Grade and Subject for 2018'))
```
...
Table `r tblNum(1)` shows the total number of valid EOCT student records from spring 2018 available for analysis.

```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_EOC <- table(PARCC_SGP@Data[YEAR=='2017_2018.2' & CONTENT_AREA %in% EOCT_subjects & VALID_CASE=='VALID_CASE']$CONTENT_AREA, PARCC_SGP@Data[YEAR=='2017_2018.2' & CONTENT_AREA %in% EOCT_subjects & VALID_CASE=='VALID_CASE']$GRADE)
```
```{r, results='asis', echo=FALSE, N_tableEOC}
  ...
  cat(dualTable(as.matrix(n_tbl_EOC_B), align=paste(rep('r', dim(n_tbl_EOC_B)[2]), collapse=''), 
		caption='Total Number of Valid EOCT Student Records by Subject for 2018'))
```
...
The data preparation step involves taking PARCC consortium assessment data provided by the Pearson Education Corporation and producing a single file that will be subsequently analyzed in Step 2. This process is carried out as new data becomes available after each assessment interval.  Data from the spring 2018 testing period was provided to the Center for Assessment in July 2018.  An individual comma separated values (.csv) file was provided for each state in the PARCC consortium. 
...
The Spring 2018 PARCC SGP analyses follow a work flow that includes the following four steps:
...
As an example, here is a Geometry configuration used to define a 2018 SGP analysis with a skipped year (i.e. using 2016 Grade 8 Math as a prior for 2018 Geometry):
...
As an example, Figure `r getCounter("figure")+1` shows the results for 8<sup>th</sup> grade ELA as an example of good model fit.  Figure `r getCounter("figure")+2` demonstrates minor model misfit in the Algebra I analyses that uses 2017 Geometry for its prior scores.
...
```{r, cache=TRUE, echo=FALSE, message=FALSE, include=FALSE, GOFplots}
  ...
  dat <- PARCC_SGP@Data[SGP_NORM_GROUP %in% c("2016_2017.2/ELA_7; 2017_2018.2/ELA_8", "2015_2016.2/ELA_6; 2016_2017.2/ELA_7; 2017_2018.2/ELA_8"),
  				 c("VALID_CASE", "CONTENT_AREA", "GRADE", "YEAR", "ID", "SGP", "SGP_SIMEX_RANKED", "SCALE_SCORE", "SCALE_SCORE_PRIOR", "SGP_NORM_GROUP"), with=FALSE]

	gofSGP(dat, state="PARCC", years='2017_2018.2', content_areas="ELA", use.sgp="SGP", output.format="PNG")
	gofSGP(dat, state="PARCC", years='2017_2018.2', content_areas="ELA", use.sgp="SGP_SIMEX_RANKED", output.format="PNG")

  dat <- PARCC_SGP@Data[SGP_NORM_GROUP %in% "2016_2017.2/GEOMETRY_EOCT; 2017_2018.2/ALGEBRA_I_EOCT", 
  				 c("VALID_CASE", "CONTENT_AREA", "GRADE", "YEAR", "ID", "SGP", "SGP_SIMEX_RANKED", "SCALE_SCORE", "SCALE_SCORE_PRIOR", "SGP_NORM_GROUP"), with=FALSE]

	gofSGP(dat, state="PARCC", years='2017_2018.2', content_areas='ALGEBRA_I', use.sgp="SGP", output.format="PNG")
	gofSGP(dat, state="PARCC", years='2017_2018.2', content_areas='ALGEBRA_I', use.sgp="SGP_SIMEX_RANKED", output.format="PNG")

	###  HAVE TO RENAME THE GoFit PLOTS CONTAINING A "." IN NAME !!!
	invisible(file.rename(
		"Goodness_of_Fit/ALGEBRA_I.2017_2018.2/2017_2018.2_ALGEBRA_I_EOCT;2016_2017.2_GEOMETRY_EOCT.png",
		"Goodness_of_Fit/ALGEBRA_I.2017_2018.2/2016_2017_2_ALGEBRA_I_EOCT;2015_2016_2_GEOMETRY_EOCT.png"))
	invisible(file.rename(
		"Goodness_of_Fit/ALGEBRA_I.2017_2018.2.RANKED_SIMEX/2017_2018.2_ALGEBRA_I_EOCT;2016_2017.2_GEOMETRY_EOCT.png",
		"Goodness_of_Fit/ALGEBRA_I.2017_2018.2.RANKED_SIMEX/2016_2017_2_ALGEBRA_I_EOCT;2015_2016_2_GEOMETRY_EOCT.png"))
...
The model fit plots for the unadjusted model results in all subjects are excellent with few exceptions (see Appendix A for all model fit plots).  Figure `r getCounter("figure")+1` displays the 8<sup>th</sup> grade ELA model as an exemplar of model fit, and Figure `r getCounter("figure")+2` is an Algebra I progression which exemplifies the extent to which model misfit is present in the PARCC SGP analyses.


```{r, results="asis", echo=FALSE, G8_ELA_GoF_Cohort}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/ELA.2017_2018.2/gofSGP_Grade_8.png",
			caption = "Goodness of Fit Plot for 2018 ***Uncorrected*** 8<sup>th</sup> Grade ELA:  Example of good model fit.")
```

Misfit in the Algebra I model is likely due to the relatively small cohort size (1,140 students).  The atypical course progression from Geometry to Algebra I also likely may mean the students in the cohort are atypical and may be more difficult to model (e.g. the decile score ranges in the SGP Range plot suggest the cohort is largely low achieving with a small group of high achieving outliers).  The indication of a potential ceiling effect in this plot is probably overstated given the range of scores included in the HOSS (bottom) row.  That is, the low SGPs are likely attributed to students scoring just below the HOSS, and therefore less concerning.  The possibility of ceiling and floor effects in this, and all other, analyses is investigated in greater detail in Appendix C of this report.

```{r, results="asis", echo=FALSE, IntMath2_GoF_Cohort}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/ALGEBRA_I.2017_2018.2/2016_2017_2_ALGEBRA_I_EOCT;2015_2016_2_GEOMETRY_EOCT.png",
			caption = "Goodness of Fit Plot for 2018 ***Uncorrected*** Algebra I (Geometry Prior) Progression: Example of model mis-fit.")
```
...
```{r, results="asis", echo=FALSE, G8_ELA_GoF_RSIMEX}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/ELA.2017_2018.2.RANKED_SIMEX/gofSGP_Grade_8.png",
			caption = "Goodness of Fit Plot for 2018 ***SIMEX Corrected*** 8<sup>th</sup> Grade ELA:  Example of good model fit.")
```


```{r, results="asis", echo=FALSE, IntMath2_GoF_RSIMEX}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/ALGEBRA_I.2017_2018.2.RANKED_SIMEX/2016_2017_2_ALGEBRA_I_EOCT;2015_2016_2_GEOMETRY_EOCT.png",
			caption = "Goodness of Fit Plot for 2018 ***SIMEX Corrected*** Algebra I (Geometry Prior) Progression: Example of model mis-fit.")
```
...
```{r, results='asis', echo=FALSE, Student_EOG}
	student.cor.grd <- PARCC_SGP@Data[YEAR=='2017_2018.2' & VALID_CASE=='VALID_CASE' & CONTENT_AREA %in% GL_subjects][, list(
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, Student_EOCT}
	student.cor.eoct <- PARCC_SGP@Data[YEAR=='2017_2018.2' & VALID_CASE=='VALID_CASE' & CONTENT_AREA %in% EOCT_subjects][, list(
		`$\\\rr_ {  Test Scores}$` = round(cor(SCALE_SCORE, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 2), 
		`$\\\rr_ {  SGP}$` = format(round(cor(SGP, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 2), nsmall = 2), 
...
```{r, results='asis', echo=FALSE, Student_EOCT_1}
	...
	invisible(eoct_tmp_tbl[, Most_Recent_Prior := gsub("2015 2016 2", "Spring 2016", Most_Recent_Prior)])
	invisible(eoct_tmp_tbl[, Most_Recent_Prior := gsub("2016 2017 2", "", Most_Recent_Prior)])
	invisible(eoct_tmp_tbl[, Most_Recent_Prior := gsub("2017 2018 1", "Fall 2017", Most_Recent_Prior)])
...
```{r, results='asis', echo=FALSE, SumEOG}
	EOG_smry <- PARCC_SGP@Data[CONTENT_AREA %in% GL_subjects & YEAR=='2017_2018.2'][, list(MEDIAN = median(as.numeric(SGP), na.rm=TRUE), MEAN = round(mean(SGP, na.rm=TRUE), 1)), by=c('CONTENT_AREA', 'GRADE')][!is.na(MEDIAN)]
  ...
  cat(dualTable(as.matrix(EOG_smryB), title="", n.cgroup=c(1, dim(EOG_smryB)[2]-1), cgroup=c("", "Grades"),
  	align=paste(rep('r', dim(EOG_smryB)[2]), collapse=''),
		caption='Spring 2018 Grade Level Median (Mean) Student Growth Percentile by Grade and Content Area.'))
```
...
The notable exception to this is 10<sup>th</sup> Grade ELA.  In this case the deviation from 50 is due to aggregation of SGPs across two different analyses: the traditional course progression of grades 8, 9 and 10 and a second analysis that ignores the 9<sup>th</sup> grade test score.  Although all PARCC students were used in the skipped grade analysis, states that use these SGPs for their students had higher typical SGPs (60+) relative to the entire PARCC cohort.  Since only these states' have these SGPs included in  and this is reflected in 



```{r, results='asis', echo=FALSE, SumEOC}
	EOC_smry <- PARCC_SGP@Data[CONTENT_AREA %in% EOCT_subjects & YEAR=='2017_2018.2'][,list("Median SGP"=median(as.numeric(SGP), na.rm=TRUE), "Mean SGP"=round(mean(SGP, na.rm=TRUE), 1)), by='CONTENT_AREA']
	setkey(EOC_smry)
  ...
  cat(dualTable(as.matrix(EOC_smry_B), align='rcc',
		caption='Spring 2018 EOCT Median and Mean Student Growth Percentile by Content Area.'))
```
...
```{r, results='asis', echo=FALSE, SIMEX_SumEOG}
  ...
  cat(dualTable(as.matrix(EOG_SIMEX_smryB), title="", n.cgroup=c(1, dim(EOG_SIMEX_smryB)[2]-1), cgroup=c("", "Grades"),
  	align=paste(rep('r', dim(EOG_SIMEX_smryB)[2]), collapse=''),
		caption='SIMEX Corrected Grade Level Median (Mean) Student Growth Percentile by Grade and Content Area for Spring 2018'))
```


```{r, results='asis', echo=FALSE, SIMEX_SumEOC}
	EOC_SIMEX_smry <- PARCC_SGP@Data[CONTENT_AREA %in% EOCT_subjects & YEAR=='2017_2018.2'][,list("Median SGP"=median(as.numeric(SGP_SIMEX_RANKED), na.rm=TRUE), "Mean SGP"=round(mean(SGP_SIMEX_RANKED, na.rm=TRUE), 1)), by='CONTENT_AREA']
  ...
  cat(dualTable(as.matrix(EOC_SIMEX_smry_B), align='rcc',
		caption='SIMEX Corrected EOCT Median (Mean) Student Growth Percentile by Content Area for Spring 2018'))
```
...
```{r, results='asis', echo=FALSE, State_Summary}
	...
	set.seed(719)
	levels(smry.grd$STATE) <- sample(LETTERS[1:6])

	###  2018  ###
	
	#  table(smry.grd[, StateAbbreviation, STATE])
	#     StateAbbreviation
	# STATE BI DC IL MD NJ NM
	#     D 24  0  0  0  0  0
	#     B  0 22  0  0  0  0
	#     A  0  0 19  0  0  0
	#     F  0  0  0 29  0  0
...
```{r, results='asis', echo=FALSE, State_Smry_ELA}
	tmp.tbl <- smry.grd[CURNT_CONTENT_AREA == "ELA" & CURNT_YEAR == '2017_2018.2' & N > 999][, 
	...
	cat(dualTable(as.matrix(tmp.tbl1), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''),
		caption='Spring 2018 Typical Prior Achievement and Growth by State: ELA', n.rgroup=tbl1.rgroup))

	cat(dualTable(as.matrix(tmp.tbl2), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''),
		caption='Spring 2018 Typical Prior Achievement and Growth by State: ELA (<em>Continued</em>)', n.rgroup=tbl2.rgroup))
```
...
```{r, results='asis', echo=FALSE, State_Smry_MATH}
	tmp.tbl <- smry.grd[CURNT_CONTENT_AREA == "MATHEMATICS" & CURNT_YEAR == '2017_2018.2' & N > 999][, 
			list(Grade = as.numeric(CURNT_GRADE), State = as.character(STATE), `Mean Prior\n Scale Score` = round(Prior_SS, 1), 
					 `Mean Prior\n Theta Score` = round(Prior_TS, 2), `PARCC %ile\n (Mean Prior Theta)` = round(PRIOR_ACHIEVEMENT_PERCENTILE, 0), 
					 `Mean SGP` = round(Mean_SGP, 1), `Mean SIMEX` = round(Mean_SIMEX, 1))]
	setkey(tmp.tbl, Grade, State)
	...
	cat(dualTable(as.matrix(tmp.tbl1), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''),
		caption='Spring 2018 Typical Prior Achievement and Growth by State: Mathematics', n.rgroup=tbl1.rgroup))

	cat(dualTable(as.matrix(tmp.tbl2), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''), n.rgroup=tbl2.rgroup,
		caption='Spring 2018 Typical Prior Achievement and Growth by State: Mathematics (<em>Continued</em>)'))
```


```{r, results='asis', echo=FALSE, State_Smry_ALG1}
	tmp.tbl <- smry.grd[CURNT_CONTENT_AREA == "ALGEBRA_I" & CURNT_YEAR == '2017_2018.2' & N > 999][, 
	...
	invisible(tmp.tbl[, Most_Recent_Prior := gsub("2016 2017 2", "", Most_Recent_Prior)])
	...
	cat(dualTable(as.matrix(tmp.tbl), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''), n.rgroup=tbl.rgroup,
		caption='Spring 2018 Typical Prior Achievement and Growth by State: Algebra I'))
```

```{r, results='asis', echo=FALSE, State_Smry_ALG2}
	tmp.tbl <- smry.grd[CURNT_CONTENT_AREA == "ALGEBRA_II" & CURNT_YEAR == '2017_2018.2' & N > 999][, 
			list(Most_Recent_Prior = Most_Recent_Prior, State = as.character(STATE), `Mean Prior\n Scale Score` = round(Prior_SS, 1), 
					 `Mean Prior\n Theta Score` = round(Prior_TS, 2), `PARCC %ile\n (Mean Prior Theta)` = round(PRIOR_ACHIEVEMENT_PERCENTILE, 0), 
					 `Mean SGP` = round(Mean_SGP, 1), `Mean SIMEX` = round(Mean_SIMEX, 1))]
	...
	invisible(tmp.tbl[, Most_Recent_Prior := sapply(gsub("/", " ", Most_Recent_Prior), capwords)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub("2016 2017 2", "", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub("2017 2018 1", "Fall 2016", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub("Mathematics", "Math", Most_Recent_Prior)])
	...
	##  NOTE - tmp.tbl[-7] to remove row with only one state
	cat(dualTable(as.matrix(tmp.tbl[-7]), title="", align=paste(rep('r', ncol(tmp.tbl)), collapse=''), n.rgroup=tbl.rgroup[tbl.rgroup>1],
		caption='Spring 2018 Typical Prior Achievement and Growth by State: Algebra II'))
```

```{r, results='asis', echo=FALSE, State_Smry_GEOM}
	tmp.tbl <- smry.grd[CURNT_CONTENT_AREA == "GEOMETRY" & CURNT_YEAR == '2017_2018.2' & N > 999][, 
			list(Most_Recent_Prior = Most_Recent_Prior, State = as.character(STATE), `Mean Prior\n Scale Score` = round(Prior_SS, 1), 
					 `Mean Prior\n Theta Score` = round(Prior_TS, 2), `PARCC %ile\n (Mean Prior Theta)` = round(PRIOR_ACHIEVEMENT_PERCENTILE, 0), 
					 `Mean SGP` = round(Mean_SGP, 1), `Mean SIMEX` = round(Mean_SIMEX, 1))]
	...
	##  NOTE - tmp.tbl[1:4] to remove rows with only one state
	cat(dualTable(as.matrix(tmp.tbl[1:4]), title="", align=paste(rep('r', ncol(tmp.tbl)), collapse=''), n.rgroup=tbl.rgroup[tbl.rgroup>1],
		caption='Spring 2018 Typical Prior Achievement and Growth by State: Geometry'))
```
