...
subtitle: "A Technical Overview of the Fall 2017 Student Growth Percentile Calculations"
author:
...
abstract: "DRAFT REPORT - DO NOT CITE! <br></br> This report provides details about the PARCC consortium Student Growth Model methodology and presents a descriptive analysis of the 2017 SGP calculation process and results."
---
...
Table `r tblNum(1)` shows the total number of valid EOCT student records from fall 2017 available for analysis.
...
The Fall 2017 PARCC SGP analyses follow a work flow that includes the following four steps:
...
SGP analyses were conducted at both the consortium and state level.  In the consortium analyses we use the `updateSGP` function to ***a)*** do the final preparation and addition of the cleaned and formatted Fall 2017 data to an `SGP` class object created for the Fall 2016 analyses (`prepareSGP` step) and ***b)*** calculate SGP estimates (`analyzeSGP` step).  In the individual state analyses, these two steps were performed using the individual functions separately in order to establish the states' `SGP` class objects (i.e. Fall state-level analyses were not feasible due to small cohort sizes).
...
```{r, results='asis', echo=FALSE, SumEOG}
  ...
  cat(dualTable(as.matrix(EOG_smryB), title="", n.cgroup=c(1, dim(EOG_smryB)[2]-1), cgroup=c("", "Grades"),
  	align=paste(rep('r', dim(EOG_smryB)[2]), collapse=''),
		caption='Fall 2017 Grade Level Median (Mean) Student Growth Percentile by Grade and Content Area.'))
```
...
```{r, results='asis', echo=FALSE, SumEOC}
  ...
  cat(dualTable(as.matrix(EOC_smry_B), align='rcc',
		caption='Fall 2017 EOCT Median and Mean Student Growth Percentile by Content Area.'))
```
...
```{r, results='asis', echo=FALSE, SIMEX_SumEOG}
  ...
  cat(dualTable(as.matrix(EOG_SIMEX_smryB), title="", n.cgroup=c(1, dim(EOG_SIMEX_smryB)[2]-1), cgroup=c("", "Grades"),
  	align=paste(rep('r', dim(EOG_SIMEX_smryB)[2]), collapse=''),
		caption='SIMEX Corrected Grade Level Median (Mean) Student Growth Percentile by Grade and Content Area for Fall 2017'))
```
...
```{r, results='asis', echo=FALSE, SIMEX_SumEOC}
  ...
  cat(dualTable(as.matrix(EOC_SIMEX_smry_B), align='rcc',
		caption='SIMEX Corrected EOCT Median (Mean) Student Growth Percentile by Content Area for Fall 2017'))
```
...
```{r, results='asis', echo=FALSE, State_Smry_ELA}
	...
	cat(dualTable(as.matrix(tmp.tbl1), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''),
		caption='Fall 2017 Typical Prior Achievement and Growth by State: ELA', n.rgroup=tbl1.rgroup))

	cat(dualTable(as.matrix(tmp.tbl2), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''),
		caption='Fall 2017 Typical Prior Achievement and Growth by State: ELA (<em>Continued</em>)', n.rgroup=tbl2.rgroup))
```
...
```{r, results='asis', echo=FALSE, State_Smry_MATH}
	...
	cat(dualTable(as.matrix(tmp.tbl1), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''),
		caption='Fall 2017 Typical Prior Achievement and Growth by State: Mathematics', n.rgroup=tbl1.rgroup))

	cat(dualTable(as.matrix(tmp.tbl2), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''), n.rgroup=tbl2.rgroup,
		caption='Fall 2017 Typical Prior Achievement and Growth by State: Mathematics (<em>Continued</em>)'))
```
...
```{r, results='asis', echo=FALSE, State_Smry_ALG1}
	...
	cat(dualTable(as.matrix(tmp.tbl), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''), n.rgroup=tbl.rgroup,
		caption='Fall 2017 Typical Prior Achievement and Growth by State: Algebra I'))
```
...
```{r, results='asis', echo=FALSE, State_Smry_ALG2}
	...
	cat(dualTable(as.matrix(tmp.tbl), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''), n.rgroup=tbl.rgroup,
		caption='Fall 2017 Typical Prior Achievement and Growth by State: Algebra II'))
```
...
```{r, results='asis', echo=FALSE, State_Smry_GEOM}
	...
	cat(dualTable(as.matrix(tmp.tbl), title="", align=paste(rep('r', dim(tmp.tbl)[2]), collapse=''), n.rgroup=tbl.rgroup,
		caption='Fall 2017 Typical Prior Achievement and Growth by State: Geometry'))
```
