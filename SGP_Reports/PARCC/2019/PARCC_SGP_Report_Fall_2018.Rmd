...
subtitle: "A Technical Overview of the Fall 2018 Student Growth Percentile Calculations"
author:
...
This document was written by Damian Betebenner & Adam VanIwaarden for the PARCC consortium and Pearson Education.
	...
	Original Draft:  May 13, 2019
	Final Draft:     May 31, 2019
-->


```{r, echo=FALSE, include=FALSE}
  ## set a universal Cache path
  knitr::opts_chunk$set(cache.path = "_cache/PARCC_SGP_Fall2018")
...
- ***Analytics - *** introduces some of the basic statistical methods and the computational process implemented in the Fall 2018 analyses.^[More in-depth treatment of the SGP Methodology can be found [here](https://github.com/CenterForAssessment/SGP_Resources/tree/master/articles) and in Appendix B of this report.]
...
- ***SGP Results - *** provides basic descriptive statistics from the Fall 2018 analyses at the consortium and state levels.

This report also includes multiple appendices.  Appendix A displays Goodness of Fit plots for each analysis conducted for Fall 2018.  Appendix B provides a more comprehensive discussion of the SGP methodology.  Appendix C is an investigation of potential ceiling and/or floor effects present in the PARCC consortium assessment data and growth analyses.
...
Data for the PARCC grade level and end-of-course tests (EOCT) used in the SGP analyses were supplied by Pearson to the NCIEA for analysis in early February 2019.  These PARCC test records were added to PARCC assessment program data from previous test administrations to create the longitudinal data set from which the Fall 2018 SGPs were calculated.  Subsequent SGP analyses will augment this multi-year data set allowing Pearson to maintain a comprehensive longitudinal data set for all students taking the PARCC assessments.

Student Growth Percentiles have been produced for students that have a current score and at least one prior score in either the same subject or a related content area.  For the Fall 2018 testing period SGPs were produced for high school English Language Arts (ELA), as well as for the Algebra I, Algebra II and Geometry EOCT subjects.  There were no viable Integrated Mathematics analyses for the Fall 2018 data.
...
Table `r tblNum(1)` shows the number of valid student records from Fall 2018 available for analysis after applying a set of general business rules.  The final raw data submissions provided by Pearson contained no duplicate records or other problematic cases, and therefore these records are all considered valid for use in the SGP models.^[This number does not represent the number of SGPs produced, however, because students are required to have at least one prior score available as well.]


```{r, cache=TRUE, echo=FALSE, include=FALSE}
n_tbl <- data.table(table(PARCC_SGP@Data[YEAR=='2018_2019.1' & VALID_CASE=='VALID_CASE', CONTENT_AREA, GRADE]))[N != 0]
n_tbl[,CONTENT_AREA := factor(CONTENT_AREA, levels=subject_order)]
n_tbl[,GRADE := as.numeric(GRADE)]
setcolorder(n_tbl, c(2,1,3))
setkey(n_tbl, CONTENT_AREA, GRADE)
n_tbl[, CONTENT_AREA := sapply(CONTENT_AREA, capwords)]
n_tbl[duplicated(CONTENT_AREA), CONTENT_AREA := ""]
setnames(n_tbl, sapply(names(n_tbl), capwords))
```
```{r, results='asis', echo=FALSE, N_tableEOC}
	n_tbl[, N := prettyNum(N, preserve.width = "individual",big.mark=',')]

  cat(dualTable(as.matrix(n_tbl), align=paste(rep('r', dim(n_tbl)[2]), collapse=''), 
		caption='Total Number of Valid PARCC Student Records by Subject and Grade for Fall 2018'))
```
...
The data preparation step involves taking PARCC consortium assessment data provided by the Pearson Education Corporation and producing a single file that will be subsequently analyzed in Step 2. This process is carried out as new data becomes available after each assessment interval.  Data from the Fall 2018 testing period was provided to the Center for Assessment in February 2019.  An individual comma separated values (.csv) file was provided for each state in the PARCC consortium that administered tests in the Fall of 2018. 

Each state specific file was read into `R` and simultaneously combined into a single data table.  The data was then cleaned up variable by variable to ensure that it conforms to data naming and variable format conventions used in the `SGP` software package.  The result is a `.Rdata` file containing data in the format suitable for analysis with the `SGP` package.  With an appropriate longitudinal data set prepared, we continued on with the calculation of student-level SGPs.
...
The Fall 2018 PARCC SGP analyses follow a work flow that includes the following four steps:

1. Create SGP configurations for analyses.
2. Conduct all grade level and EOCT SGP analyses.
3. Combine results into the master longitudinal data set and output data in the format requested by Pearson.
...
Configurations are `R` code scripts that are used to define each SGP analysis.  They are broken up into four separate `R` scripts based on content domain (ELA and mathematics) and test score metric (IRT theta and scaled scores).  Each configuration code chunk specifies a set of parameters that identifies the norm group of students to be examined.  Every potential norm group is defined by, at a minimum, the progressions of content area, academic year and grade-level.  Therefore, every configuration used contains the first three elements and some analyses also contain the fourth through sixth elements:

- **`sgp.content.areas`:** The ordered progression of content areas to be looked at.
- **`sgp.panel.years`:** The progression of the years associated with the content area progression (`sgp.content.areas`), potentially allowing for skipped or repeated years, fall to spring (within academic year) analyses, etc.  Because some PARCC states administer tests in both Fall and Spring, a season or period indicator is required as well.  This is encoded here as either a ".1" or ".2" suffix to the academic year (i.e. "2018_2019.1" for Fall 2018 and "2018_2019.2" for Spring 2019). 
- **`sgp.grade.sequences`:** The grade progression associated with the configuration content areas and years. The value **'EOCT'** stands for 'End Of Course Test'.  The use of the generic 'EOCT' label allows for secondary students to be compared based on the pattern of course taking rather than being dependent upon grade-level designation.
...
In previous years' this meta-data was encoded into a lookup table included in the `SGPstateData`, However, beginning in 2018, the `sgp.norm.group.preference` element of the analyses configuration code has been used to directly incorporate the preferencing system.
...
SGP analyses were conducted at both the consortium and state level.  For all analyses we use the `abcSGP` function to complete all data management, analytic and visualization steps.  In the `prepareSGP` step the final preparation and addition of the cleaned and formatted Fall 2018 data to prior years data is completed at which point we can calculate SGP estimates (`analyzeSGP` step).
...
```{r, cache=TRUE, echo=FALSE, message=FALSE, include=FALSE, GOFplots}
  ...
  data.table::setkeyv(PARCC_SGP@Data, SGP:::getKey(PARCC_SGP))

	### ELA Grade 9 as example of GOOD fit ...
  ##  Choice of 9th grade and ELA are arbitrary ...
  ##  Keep using that from year to year to show it IS arbitrary - no need to change year to year ... ...

  dat <- PARCC_SGP@Data[SGP_NORM_GROUP %in% c("2017_2018.2/ELA_8; 2018_2019.1/ELA_9", "2016_2017.2/ELA_7; 2017_2018.2/ELA_8; 2018_2019.1/ELA_9"),
  				 c("VALID_CASE", "CONTENT_AREA", "GRADE", "YEAR", "ID", "SGP", "SGP_SIMEX_RANKED", "SCALE_SCORE", "SCALE_SCORE_PRIOR", "SGP_NORM_GROUP"), with=FALSE] #  All 3994 cases are in @Data

	gofSGP(dat, state="PARCC", years='2018_2019.1', content_areas="ELA", use.sgp="SGP", output.format="PNG")
	gofSGP(dat, state="PARCC", years='2018_2019.1', content_areas="ELA", use.sgp="SGP_SIMEX_RANKED", output.format="PNG")

  dat <- PARCC_SGP@SGP$SGPercentiles$ALGEBRA_II.2018_2019.1[SGP_NORM_GROUP == "2017_2018.1/GEOMETRY_EOCT; 2018_2019.1/ALGEBRA_II_EOCT"]
  dat[, SCALE_SCORE := as.numeric(sapply(strsplit(as.character(SGP_NORM_GROUP_SCALE_SCORES), "; "), function(x) rev(x)[1]))]
  dat[, VALID_CASE := "VALID_CASE"]
  dat[, CONTENT_AREA := "ALGEBRA_II"]
  dat[, YEAR := "2018_2019.1"]
  
	gofSGP(dat, state="PARCC", years='2018_2019.1', content_areas='ALGEBRA_II', use.sgp="SGP", output.format="PNG")
	gofSGP(dat, state="PARCC", years='2018_2019.1', content_areas='ALGEBRA_II', use.sgp="SGP_SIMEX_RANKED", output.format="PNG")

	###  HAVE TO RENAME THE GoFit PLOTS CONTAINING A "." IN NAME !!!
	invisible(file.rename(
		"Goodness_of_Fit/ALGEBRA_II.2018_2019.1/2018_2019.1_ALGEBRA_II_EOCT;2017_2018.1_GEOMETRY_EOCT.png",
		"Goodness_of_Fit/ALGEBRA_II.2018_2019.1/2018_2019_1_ALGEBRA_II_EOCT;2017_2018_1_GEOMETRY_EOCT.png"))
	invisible(file.rename(
		"Goodness_of_Fit/ALGEBRA_II.2018_2019.1.RANKED_SIMEX/2018_2019.1_ALGEBRA_II_EOCT;2017_2018.1_GEOMETRY_EOCT.png",
		"Goodness_of_Fit/ALGEBRA_II.2018_2019.1.RANKED_SIMEX/2018_2019_1_ALGEBRA_II_EOCT;2017_2018_1_GEOMETRY_EOCT.png"))
...
The model fit plots for the unadjusted model results in all subjects are generally good given the relatively small cohort sizes of the Fall 2018 assessments (see Appendix A for all model fit plots).  Figure `r getCounter("figure")+1` displays the 9<sup>th</sup> grade ELA model as an exemplar of model fit, and Figure `r getCounter("figure")+2` is an Algebra II progression which exemplifies the extent to which model misfit is present in the PARCC SGP analyses.


```{r, results="asis", echo=FALSE, G9_ELA_GoF_Cohort}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/ELA.2018_2019.1/gofSGP_Grade_9.png",
			caption = "Goodness of Fit Plot for Fall 2018 ***Uncorrected*** 9<sup>th</sup> Grade ELA:  Example of good model fit.")
```

Misfit in the Algebra II model is likely due to the relatively small cohort size (1,298 students).  The atypical course progression from Fall 2017 to Fall 2018 may also mean the students in the cohort are atypical and may be more difficult to model.  The indication of a potential ceiling effect in this plot is probably overstated given the range of scores included in the HOSS (bottom) row.  That is, the low SGPs are likely attributed to students scoring just below the HOSS, and therefore less concerning.  The possibility of ceiling and floor effects in this, and all other, analyses is investigated in greater detail in Appendix C of this report.

```{r, results="asis", echo=FALSE, Alg2_GoF_Cohort}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/ALGEBRA_II.2018_2019.1/2018_2019_1_ALGEBRA_II_EOCT;2017_2018_1_GEOMETRY_EOCT.png",
			caption = "Goodness of Fit Plot for Fall 2018 ***Uncorrected*** Algebra I (Geometry Prior) Progression: Example of model mis-fit.")
```
...
SIMEX model fit is **not** expected to be perfect (i.e. uniformly distributed regardless of prior achievement).  In these models we ***expect misfit*** in the form of increased high SGPs for students with lower prior performance (and a complementary decrease in low SGPs for those students), and the reverse expectation for high achieving students.  This effect is alleviated to some extent through the ranking of the SIMEX SGPs [@CastMcCaf:2017].  This shift is visible in the goodness of fit plots in Figure `r getCounter("figure")+1` where the Ranked SIMEX correction method has been applied to the 9<sup>th</sup> grade ELA model, and Figure `r getCounter("figure")+2` for the same Algebra II progression as presented above.  Most notable is the shift from blue to red in the top half (and red to blue in the bottom half) of the "Student Growth Percentile Range" panel.

The patterns observed in these two charts suggest that the SIMEX model adjustments are behaving as expected.

<p></p>

```{r, results="asis", echo=FALSE, G9_ELA_GoF_RnkSIMEX}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/ELA.2018_2019.1.RANKED_SIMEX/gofSGP_Grade_9.png",
			caption = "Goodness of Fit Plot for Fall 2018 ***SIMEX Corrected*** 9<sup>th</sup> Grade ELA:  Example of good model fit.")
```


```{r, results="asis", echo=FALSE, Alg2_GoF_RnkSIMEX}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/ALGEBRA_II.2018_2019.1.RANKED_SIMEX/2018_2019_1_ALGEBRA_II_EOCT;2017_2018_1_GEOMETRY_EOCT.png",
			caption = "Goodness of Fit Plot for Fall 2018 ***SIMEX Corrected*** Algebra II (Geometry Prior) Progression: Example of model mis-fit.")
```
...
SIMEX corrected SGPs induce a negative correlation between growth and prior achievement.  Rather than a uniform distribution, SIMEX produces a distribution in which growth for lower prior achieving students' is weighted upward and higher achieving students' growth is weighted down.  In theory, this bias in student-level SGPs may decrease the bias in aggregate growth measures (as documented in previous SIMEX reports and Shang et al., 2015).  Subsequently, the correlations between both student- and aggregate-level "unadjusted" SGPs and prior achievement are typically higher than those with SIMEX corrected SGPs.^[Correlations that are initially negative will become increasingly negative rather than return to zero.]


### Grade level subjects

```{r, results='asis', echo=FALSE, Student_EOG}
	student.cor.grd <- PARCC_SGP@Data[YEAR=='2018_2019.1' & VALID_CASE=='VALID_CASE' & CONTENT_AREA %in% GL_subjects][, list(
		`$\\\rr_ {  Test Scores}$` = round(cor(SCALE_SCORE, SCALE_SCORE_PRIOR, use='pairwise.complete'), 2), 
		`$\\\rr_ {  SGP}$` = format(round(cor(SGP, SCALE_SCORE_PRIOR, use='pairwise.complete'), 2), nsmall = 2), 
		`$\\\rr_ {  SIMEX SGP}$` = round(cor(SGP_SIMEX_RANKED, SCALE_SCORE_PRIOR, use='pairwise.complete'), 2), 
		N_Size = sum(!is.na(SGP))), keyby = list(CONTENT_AREA, GRADE, Most_Recent_Prior)]
	...
	invisible(gl_tmp_tbl[, Most_Recent_Prior := sapply(gsub("/", " ", Most_Recent_Prior), capwords)])
	invisible(gl_tmp_tbl[, Most_Recent_Prior := gsub("2017 2018 2", "Spring '18", Most_Recent_Prior)])
	invisible(gl_tmp_tbl[, Most_Recent_Prior := gsub("2017 2018 1", "Fall '17", Most_Recent_Prior)])
	invisible(gl_tmp_tbl[, Most_Recent_Prior := gsub("ELA ", "", Most_Recent_Prior)])
	invisible(gl_tmp_tbl[, Most_Recent_Prior := gsub(" 8", " Grade 8", Most_Recent_Prior)])
	invisible(gl_tmp_tbl[, Most_Recent_Prior := gsub(" 9", " Grade 9", Most_Recent_Prior)])
	invisible(gl_tmp_tbl[, Most_Recent_Prior := gsub(" 10", " Grade 10", Most_Recent_Prior)])
	...
	setnames(gl_tmp_tbl, c(1:3,7), sapply(names(gl_tmp_tbl)[c(1:3,7)], capwords))
...
Each EOCT subject is analyzed using more than one sequence of prior subjects, grades and years, and these unique progressions are disaggregated in Table `r tblNum(1)` using the most recent prior available for each norm group (although more prior years' scores can be used in SGP calculations when available).  Unless otherwise noted, all priors are from Spring 2018.  The correlations between current EOCT and prior test score are generally lower than in the grade level norm groups, and overall lower correlations may be expected in EOCT subjects due to the change in specific subject from one course to the next.  

The relationships between growth and prior achievement reported in Table `r tblNum(1)` are near zero for cohort referenced SGPs and slightly negative after SIMEX correction.  These results are as expected for appropriate fit to the respective models as discussed in the grade level section above.

```{r, cache=TRUE, echo=FALSE, include=FALSE, Student_EOCT}
	student.cor.eoct <- PARCC_SGP@Data[YEAR=='2018_2019.1' & VALID_CASE=='VALID_CASE' & CONTENT_AREA %in% EOCT_subjects][, list(
		`$\\\rr_ {  Test Scores}$` = round(cor(SCALE_SCORE, SCALE_SCORE_PRIOR, use='pairwise.complete.obs'), 2), 
		`$\\\rr_ {  SGP}$` = format(round(cor(SGP, SCALE_SCORE_PRIOR, use='pairwise.complete.obs'), 2), nsmall = 2), 
		`$\\\rr_ {  SIMEX}$` = round(cor(SGP_SIMEX_RANKED, SCALE_SCORE_PRIOR, use='pairwise.complete.obs'), 2), 
...
```{r, results='asis', echo=FALSE, Student_EOCT_1}
	...
	invisible(eoct_tmp_tbl[, Most_Recent_Prior := gsub("2017 2018 2", "Spring '18", Most_Recent_Prior)])
	invisible(eoct_tmp_tbl[, Most_Recent_Prior := gsub("2017 2018 1", "Fall '17", Most_Recent_Prior)])
	invisible(eoct_tmp_tbl[, Most_Recent_Prior := gsub("Mathematics", "Math", Most_Recent_Prior)])
	...
	cat(dualTable(as.matrix(eoct_tmp_tbl), align=paste(rep('r', dim(eoct_tmp_tbl)[2]), collapse=''), 
								caption = "EOCT Student Level Correlations between Prior Standardized Scale Score and 1) Current Scale Score, 2) SGP and 3) SIMEX SGP - Disaggregated by Norm Group."))
```
...
To demonstrate the norm-referenced nature of the growth percentiles viewed at the consortium level, Table `r tblNum(1)` presents the cohort-referenced growth percentile medians and means for all content areas.

```{r, results='asis', echo=FALSE, warning=FALSE, message=FALSE, SumEOG}
	EOG_smry <- PARCC_SGP@Data[YEAR=='2018_2019.1'][, list("Median SGP" = median(as.numeric(SGP), na.rm=TRUE), "Mean SGP" = round(mean(SGP, na.rm=TRUE), 1)), by=c('CONTENT_AREA', 'GRADE')][!is.na(`Median SGP`)]
	invisible(EOG_smry[, GRADE := as.numeric(GRADE)])
	setkey(EOG_smry)

	EOG_smry <- EOG_smry[][order(match(EOG_smry$CONTENT_AREA, subject_order))]

	invisible(EOG_smry[, CONTENT_AREA := sapply(CONTENT_AREA, capwords, special.words=c("ELA", "II", "US"))])
	EOG_smry$CONTENT_AREA[duplicated(EOG_smry$CONTENT_AREA)] <- ""

	setnames(EOG_smry, sapply(names(EOG_smry), capwords))

  cat(dualTable(as.matrix(EOG_smry), title="", align='rrcc', 
  	caption='Fall 2018 Median and Mean Student Growth Percentile by Grade and Content Area.'))
```
...
Simulation-extrapolation (SIMEX) methods of correcting for measurement error induced bias in the SGP estimation has been proposed and tested in other states.  This correction has been applied to the Fall 2018 SGPs in addition to a system of ranking the SIMEX SGPs in order to improve some student level properties of the growth measure [@CastMcCaf:2017].  Descriptive statistics from the uncorrected and Ranked SIMEX corrected SGP results are both presented here.  The interested reader can find more in depth discussions of the SGP methodology in the [available literature](https://github.com/CenterForAssessment/SGP_Resources/tree/master/articles), and information about the SIMEX measurement error correction methodology is available in recent academic articles [@ShangVanIBet:2015; @CastMcCaf:2017].


```{r, results='asis', echo=FALSE, warning=FALSE, message=FALSE, SIMEX_SumEOG}
	SIMEX_smry <- PARCC_SGP@Data[YEAR=='2018_2019.1'][,list("Median SGP"=median(as.numeric(SGP_SIMEX_RANKED), na.rm=TRUE), "Mean SGP" = round(mean(SGP_SIMEX_RANKED, na.rm=TRUE), 1)), by=c('CONTENT_AREA', 'GRADE')][!is.na(`Median SGP`)]
	invisible(SIMEX_smry[, GRADE := as.numeric(GRADE)])
	setkey(SIMEX_smry)

	SIMEX_smry <- SIMEX_smry[][order(match(SIMEX_smry$CONTENT_AREA, subject_order))]

	invisible(SIMEX_smry[, CONTENT_AREA := sapply(CONTENT_AREA, capwords, special.words=c("ELA", "II", "US"))])
	SIMEX_smry$CONTENT_AREA[duplicated(SIMEX_smry$CONTENT_AREA)] <- ""

	setnames(SIMEX_smry, sapply(names(SIMEX_smry), capwords))

  cat(dualTable(as.matrix(SIMEX_smry), title="", align='rrcc',
  	caption='SIMEX Corrected Median and Mean Student Growth Percentile by Grade and Content Area for Fall 2018'))
```


A comparison of the unadjusted (Table `r tblNum(-1)`) and Ranked SIMEX corrected (Table `r tblNum()`) shows very little difference in the medians and means.  This is not surprising as the majority of the growth percentiles for students in the middle of the prior score distributions change very little, and the larger changes that occur for students in the extremes of the prior score distributions tend to even out.  
...
Summary values are provided in the following tables only when a state has *250 students or greater* in a norm group.  A randomized (i.e. non-alphabetical) letter has been attached to each state in order to protect the identity of the member states, and results are shown only when there is more than one state that meets the 250 student threshold.  As in previous tables of SGP results in this report, ELA results are disaggregated by grade level and EOCT Math tables are disaggregated by the most recent prior ensuring that all SGPs included in the summary statistics include only results from one analysis (norm group).  Tables for the Geometry SGPs are not presented as only one state had a cohort with greater than 250 students, and therefore presenting this table may compromise state anonymity.
...
```{r, results='asis', echo=FALSE, State_Summary}
	...
	set.seed(719)
	levels(smry.grd$STATE) <- sample(LETTERS[1:3])
...
```{r, results='asis', echo=FALSE, State_Smry_ELA}
	tmp.tbl <- smry.grd[CURNT_CONTENT_AREA == "ELA" & CURNT_YEAR == '2018_2019.1' & N > 249][, 
			list(Grade = as.numeric(CURNT_GRADE), Most_Recent_Prior = Most_Recent_Prior, State = as.character(STATE), `Mean Prior\n Scale Score` = round(Prior_SS, 1), 
					 `Mean Prior\n Theta Score` = round(Prior_TS, 2), `PARCC %ile\n (Mean Prior Theta)` = round(PRIOR_ACHIEVEMENT_PERCENTILE, 0), 
					 `Mean\n SGP` = round(Mean_SGP, 1), `Mean\n SIMEX` = round(Mean_SIMEX, 1))]

	invisible(tmp.tbl[, Most_Recent_Prior := sapply(gsub("/", " ", Most_Recent_Prior), capwords)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub("2017 2018 2", "", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub("2017 2018 1", "Fall '17", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub("ELA ", " ", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub(" 8", " Grade 8", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub(" 9", " Grade 9", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub(" 10", " Grade 10", Most_Recent_Prior)])

	setkey(tmp.tbl, Grade, Most_Recent_Prior, State)
	...
	cat(dualTable(as.matrix(tmp.tbl2), title="", align='rrrccccc',
		caption='Fall 2018 Typical Prior Achievement and Growth by State: ELA', n.rgroup=tbl1.rgroup))
```
...
```{r, results='asis', echo=FALSE, State_Smry_ALG1}
	tmp.tbl <- smry.grd[CURNT_CONTENT_AREA == "ALGEBRA_I" & CURNT_YEAR == '2018_2019.1' & N > 249][, 
	...
	cat(dualTable(as.matrix(tmp.tbl), title="", align='rrccccc', n.rgroup=tbl.rgroup,
		caption='Fall 2018 Typical Prior Achievement and Growth by State: Algebra I'))
```

```{r, results='asis', echo=FALSE, State_Smry_ALG2}
	tmp.tbl <- smry.grd[CURNT_CONTENT_AREA == "ALGEBRA_II" & CURNT_YEAR == '2018_2019.1' & N > 249][, 
			list(Most_Recent_Prior = Most_Recent_Prior, State = as.character(STATE), `Mean Prior\n Scale Score` = round(Prior_SS, 1), 
					 `Mean Prior\n Theta Score` = round(Prior_TS, 2), `PARCC %ile\n (Mean Prior Theta)` = round(PRIOR_ACHIEVEMENT_PERCENTILE, 0), 
					 `Mean SGP` = round(Mean_SGP, 1), `Mean SIMEX` = round(Mean_SIMEX, 1))]
	...
	invisible(tmp.tbl[, Most_Recent_Prior := sapply(gsub("/", " ", Most_Recent_Prior), capwords)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub("2017 2018 2", "", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub("2017 2018 1", "Fall 2017", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub("Mathematics", "Math", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub(" Eoct", "", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub(" 8", " Grade 8", Most_Recent_Prior)])
	invisible(tmp.tbl[, Most_Recent_Prior := gsub(" 7", " Grade 7", Most_Recent_Prior)])
	...
	##  NOTE - tmp.tbl[-3] to remove row with only one state
	cat(dualTable(as.matrix(tmp.tbl[-3]), title="", align='rrccccc',
		caption='Fall 2018 Typical Prior Achievement and Growth by State: Algebra II'))
```

```{r, results='asis', echo=FALSE, State_Smry_GEOM}
	tmp.tbl <- smry.grd[CURNT_CONTENT_AREA == "GEOMETRY" & CURNT_YEAR == '2018_2019.1' & N > 249][, 
			list(Most_Recent_Prior = Most_Recent_Prior, State = as.character(STATE), `Mean Prior\n Scale Score` = round(Prior_SS, 1), 
					 `Mean Prior\n Theta Score` = round(Prior_TS, 2), `PARCC %ile\n (Mean Prior Theta)` = round(PRIOR_ACHIEVEMENT_PERCENTILE, 0), 
					 `Mean SGP` = round(Mean_SGP, 1), `Mean SIMEX` = round(Mean_SIMEX, 1))]
	...
	##  NOTE - ONLY 1 State with 250+ students - don't show table
	# cat(dualTable(as.matrix(tmp.tbl[1:4]), title="", align=paste(rep('r', ncol(tmp.tbl)), collapse=''), n.rgroup=tbl.rgroup[tbl.rgroup>1],
		# caption='Fall 2018 Typical Prior Achievement and Growth by State: Geometry'))
```

Only one state had more than 250 students with Fall 2018 Geometry SGPs, therefore that table is omitted from this report. 
