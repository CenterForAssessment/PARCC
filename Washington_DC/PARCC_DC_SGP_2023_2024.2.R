...
###############################################################################
###                                                                         ###
###          Washington_DC 2024 (Cohort and Baseline) SGP Analyses          ###
###                                                                         ###
###############################################################################
...
###   Use the "2023" Single prior baselines per OSSE email 7/12/24

load("./Data/Archive/Washington_DC_Baseline_Matrices-2023.Rdata")
...
load("./Data/Archive/2023_2024.2/Washington_DC_Data_LONG_2023_2024.2.Rdata")
...
table(Washington_DC_Data_LONG$YEAR)
...
DC_Config_2024 <-
    c(ELA.2024.config,
      ...
      # ALGEBRA_II.2024.config # Do not add when calculating baseline growth concurrently
    )
...
Washington_DC_SGP@Version$session_platform <- list("2024" = session_platform)
Washington_DC_SGP@Version$attached_pkgs <- list("2024" = attached_pkgs)
Washington_DC_SGP@Version$namespace_pkgs<-  list("2024" = namespace_pkgs)

save(Washington_DC_SGP, file = "./Data/Archive/2023_2024.2/Washington_DC_SGP.Rdata")
...
#' baseline-referenced SGPs were calculated concurrently for the 2024 Washington DC
...
#'
#' The 2024 SGP analyses were conducted at both the consortium and member/state
...
#' analyses for 2024 with DC only data, although those students with Algebra 2
#' scores may have had growth calculated at the Consortium level. The 2024
#' baseline SGP analysis versions use a single prior year score (2023 where
#' available) as the baseline norm groups are the 2023 cohorts, which only have
#' as single prior available since DC assessments did not resume until 2022.
#' The 2024 cohort-referenced SGP analyses use up to two prior years' scores
#' (2022 and 2023) where available.
#'
