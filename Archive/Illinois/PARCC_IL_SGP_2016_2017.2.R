...
ELA.2016_2017.2.config[[1]]$sgp.grade.sequences <- list(c("3", "4"), c("3", "4", "5"), c("4", "5", "6"), c("5", "6", "7"), c("6", "7", "8"))
ELA_SS.2016_2017.2.config[[1]]$sgp.grade.sequences <- list(c("3", "4"), c("3", "4", "5"), c("4", "5", "6"), c("5", "6", "7"), c("6", "7", "8"))
...
PARCC_2016_2017.2.config <- c(
	...
	ALGEBRA_I_SS.2016_2017.2.config)#, # Only 7th grade math to Algebra I

PARCC_2016_2017.2.config <- PARCC_2016_2017.2.config[sapply(PARCC_2016_2017.2.config, function(f) any(grepl(paste("ELA", "MATHEMATICS", sep="|"), f)))]
...
Illinois_SGP <- abcSGP(
		state="IL",
		sgp_object=fetchPARCC(state="IL", parcc.db = "../PARCC/Data/PARCC_Data_LONG.sqlite", prior.years=c("2015_2", "2016_1", "2016_2", "2017_1"), current.year="2017_2", fields="*")[CONTENT_AREA %in% c("ELA", "MATHEMATICS", "ALGEBRA_I", "ELA_SS", "MATHEMATICS_SS", "ALGEBRA_I_SS")],
...
### Save results
if (sgp.test) {
	save(Illinois_SGP, file="./Data/SIM/Illinois_SGP-Test_PARCC_2016_2017.2.Rdata")
} else save(Illinois_SGP, file="./Data/Archive/2016_2017.2/Illinois_SGP.Rdata")
...
New_Jersey_SGP <- analyzeSGP(
		...
		parallel.config = list(BACKEND="PARALLEL", WORKERS=list(PROJECTIONS = workers-3, LAGGED_PROJECTIONS = workers/3)))
