...
if [ "$sgp_test" = "sgp.test <- TRUE" ]; then
	...
	echo Beginning Part 1 - Spring 15 to Fall 15, SGP object creation
...
if [ "$sgp_test" = "sgp.test <- TRUE" ]; then
	...
	wait $pid
	echo Running PARCC with TEST cohort subset
	echo Beginning Part 2 - Spring 15 to Spring 16
	...
	echo Beginning Spring 15 to Spring 16 Analyses
